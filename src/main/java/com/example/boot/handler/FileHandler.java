...
public class FileHandler {
    // 파일저장 + DB에 들어갈 List 생성
    private final String UP_DIR = "C:\\java\\_myProject\\_java\\_fileUpload";
    ...
    public List<FileDTO> uploadFile(MultipartFile[] files){
        ...
        // mkdir(폴더 1개만 생성) / mkdirs(하위 폴더까지 동시 생성)
        if (!folders.exists()){
            folders.mkdirs();
        }
        ...
        for (MultipartFile file : files){
            // file : name, size, type, saveDir=today
            ...
            fileDTOList.add(fileDTO); // fileDTO 리스트에 추가
            // ---------------------------------------------------------
